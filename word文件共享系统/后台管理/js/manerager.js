$(document).ready(function function_name (argument) {
	...
	/*科目设置中的事件*/
	$(".lession_mandiv .addiocn").on("click",function(){
      if($(this).hasClass("active"))
      {
         $(this).next(".addlessstypediv").animate({width: 'hide'},200)
         $(this).removeClass("active").html("+")
      }
      else
      {
      $(this).next(".addlessstypediv").animate({width: 'show'},500)
      $(this).addClass("active").html("X")
    	}
   	})
   	$(".lession_mandiv .addlesstype_text").on("keyup",function(){
      var a=$.trim($(this).val())
      if(a.length>2 && a.length<15)
      {
         $(this).next(".addlestype_but").addClass("ok");
      }
      else
      {
         $(this).next(".addlestype_but").removeClass("ok");
      }
   	})
   	$(".lession_mandiv .addlestype_but").on("click",function(){
      if($(this).hasClass("ok")){
         var a=$.trim($(this).prev("input").val());
         var value=ajax('post','ajax/man-tea.aspx',{'type':'6','科目类型名称':a},false)
...
function leftmenu_click()
{
	...
	else
	{
		...
		if(a=="5")
		{
			$(".right_condiv.lession_mandiv").removeClass("hiden")
			loadlession()
		}
	}
}
...
function load_classliat(dom)
{
	...
	else
	{
		...
		$(".addClass_tr .addClass_but").click('click',function(){
			if($(this).hasClass("ok"))
			{
				var year=dom1.find(".addClass_tr .addgread_select").children("option:selected").attr("value");
				if(year=="null")
				{
					alert("请选择年级");
				}
...
function loadinstrinfo(dom,a,b)
{
	...
	if(value.state=="success")
	{
		...
		$(value.data).each(function(){
			...
			html+="<option value='null'>不选择</option>"
...
function loadclasslist(a,b,c)
{
	...
	if (value.state=="success") {
		...
		$(value.data).each(function(){
			...
			html+="<td class='ctd4'><div class='full instruct ";
			html+=this.姓名==""?"error' value='error'":"' value='"+this.辅导员编号+"'";
			html+="'>";
			html+=this.姓名==""?'未分配':this.姓名;
			html+="</div><select class='changeinst_select full hiden'><select></td>";
			html+="<td class='ctd5'>";
			html+=this.数量==""?"0":this.数量;
			html+="</td>";
			html+="<td class='ctd6'><input class='changeclass' id='"+this.班级编号+"' type='button' value='修改' /></td>"
			html+="</tr>"
		})

		a.html(html);
		a.find(".instruct").on("dblclick",function(){
			var dom =$(this).next(".changeinst_select");
			dom.removeClass("hiden");
			$(this).addClass("hiden");
			var a1=$(this).attr("value");
			var b1=$(this).html();
			loadinstrinfo(dom,a1,b1)

			dom.change(function(){
				var instruid=$(this).children("option:selected").attr("value");
				if(instruid=="error")
				{
					$(this).parent("td").parent("tr").find(".changeclass").removeClass("ok");
				}
				else
				{
					$(this).parent("td").parent("tr").find(".changeclass").addClass("ok")

				}
			})
		})

		a.find(".changeclass").on("click",function(){
			if($(this).hasClass("ok"))
			{
				var instruid=a.find(".changeinst_select").children("option:selected").attr("value");
				var classid=$(this).attr("id");
				var data={'type':'23','辅导员编号':instruid,'班级编号':classid};
				var value1=ajax('post','ajax/manerager.aspx',data,false);
				if(value1.state=="success")
				{
					alert("修改成功！");
					loadclasslist(a,b,c)
